...
YELLOW = "\033[1;93m"
RESET = "\033[0;0m"
COL = {"3.3v": LRED, "5v": RED, "GND": GREEN}
...
def pin_state(g):
    ...
    result = subprocess.run(
        ["raspi-gpio", "get", ascii(g)], stdout=subprocess.PIPE
    ).stdout.decode("utf-8")
...
def print_gpio(pin_state):
    ...
    for h in range(1, GPIOPINS, 2):
        # odd pin
        hh = HEADER[h - 1]
        if type(hh) is type(1):
            print(
                "|{0:4} | {1[0]:<10} | {1[1]:<4} | {1[2]} |{2:3} ".format(
                    hh, pin_state(hh), h
                ),
                end="|| ",
            )
        ...
        if type(hh) is type(1):
            ...
            print(
                "{}{:2} |             {:9}{}      |".format(COL[hh], h + 1, hh, RESET)
            )  # coloured output
    print("+-----+------------+------+---+----++----+---+------+-----------+-----+")
    print("| BCM |    Name    | Mode | V |  Board   | V | Mode | Name      | BCM |")
    print("+-----+------------+------+---+{:^10}+---+------+-----------+-----+".format(Model))
